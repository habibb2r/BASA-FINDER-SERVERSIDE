...
rentalHouseSchema.statics.isRentalHouseAvailable = async function (id: string) {
  ...
  if (!rentalHouse) {
    throw new AppError(
      ...
      'Rental house not found or not available',
    );
  }
  return rentalHouse;
};

rentalHouseSchema.statics.isRentalHouseExists = async function (
  location: string,
  landlordId: string,
) {
  const rentalHouse = await this.findOne({
...
export const RentalHouse = model<TRentalHouse, RentalHouseModel>(
  'RentalHouse',
  rentalHouseSchema,
);
