...
const createRentalHouseInDB = async (rentalHouseData: TRentalHouse) => {
  ...
  if (
    await RentalHouse.isRentalHouseExists(
      rentalHouseData.location,
      rentalHouseData.landlordId,
    )
  ) {
    throw new AppError(
      httpStatus.CONFLICT,
      'You already have a rental house listing at this location',
    );
  }
...
const deleteRentalHouseFromDB = async (
  ...
  if (userRole !== USER_ROLE.admin && rentalHouse.landlordId !== userId) {
    throw new AppError(
      ...
      "You don't have permission to delete this rental house",
    );
  }
...
const updateRentalHouseInDB = async (
  id: string,
  updateData: Partial<TRentalHouse>,
  userId: string,
  userRole: string,
) => {
  const rentalHouse = await RentalHouse.findById(id);
  ...
  if (userRole !== USER_ROLE.admin && rentalHouse.landlordId !== userId) {
    ...
    throw new AppError(
      ...
      "You don't have permission to update this rental house",
    );
  }

  // Prevent landlordId change
  if (
    updateData.landlordId &&
    updateData.landlordId !== rentalHouse.landlordId
  ) {
